...
For applications running in Kubernetes with
[Azure Workload Identity](https://learn.microsoft.com/en-us/entra/workload-id/workload-identity-federation),
you can use federated credentials instead of client secrets. This eliminates
the need to store and rotate secrets in your Kubernetes deployments.

### Setup

1. Configure your App Registration with a federated credential pointing to your
   Kubernetes service account.

2. Enable the Azure Workload Identity webhook on your Kubernetes cluster.

3. Annotate your service account and pods to use workload identity.

### Configuration

Use the `clientAssertion` option to provide the federated token for
authentication. The token file path is injected by the workload identity
webhook via the `AZURE_FEDERATED_TOKEN_FILE` environment variable.
...
export const { handle, signIn, signOut } = SvelteKitAuth({
  providers: [
    MicrosoftEntraID({
      ...
      // Use workload identity in Kubernetes, client secret for local dev
      clientAssertion: env.AZURE_FEDERATED_TOKEN_FILE
        ? () => readFile(env.AZURE_FEDERATED_TOKEN_FILE, "utf8")
        : undefined,
...
In Kubernetes with workload identity enabled, the mutating webhook
automatically injects these environment variables:
...
For local development, you can use a client secret instead by setting
`AZURE_CLIENT_SECRET` in your environment.

<Callout type="info">
  When `clientAssertion` is provided, it takes precedence over `clientSecret`
  for the token exchange. You can provide both options to support different
  environments (workload identity in production, client secret locally).
</Callout>

### Resources

- [Azure Workload Identity documentation](https://learn.microsoft.com/en-us/entra/workload-id/workload-identity-federation)
